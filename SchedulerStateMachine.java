    ...
    public Job pressFloorButton(Job job){
        ...
        switch(currentState){
            case WAIT_FOR_FLOORSUBSYTEM:
                ...
                System.out.println("Scheduler is ready to receive job from floor subsystem");
                break;
            case RECEIVE_EVENT:
                currentState = SchedulerState.SEND_EVENT;
                scheduler.put(job);
                System.out.println("Scheduler is ready to send job to elevator subsystem");
                break;
            case SEND_EVENT:
                currentState = SchedulerState.WAIT_FOR_FLOORSUBSYTEM;
                getJob = scheduler.get();
                System.out.println("Scheduler is waiting for job request from floor subsystem");
                break;
        }
